...
/* ══ BEHAVIOURS ══ */
/* Boot: start the habit data subscriptions. Nothing else — this used to wire a
   page of its own, whose markup 14-habits-x.js overwrites on first render. The
   subscriptions stay here because the dashboard, Insights and the goal rollups
   all read _habits and _habitLogs whether or not the Habits page is open. */
...
function _tdInit(){
  ...
  /* One dull white line at an even weight the whole way round -- no gradient,
     nothing to read as a bar at either end -- and time fills it with the
     glowing line as it passes. The filled length is the elapsed session. */
  const track=mkArc(1.6,0.16), fill=mkArc(3,0.9);
  gOver.appendChild(sg);
  _tdSessionEls={root:sg,track,fill};
...
function _tdDrawSession(){
  ...
  // The head starts at the left end and travels right as the session burns down,
  // so the glow grows behind it in the same direction the timeline below reads.
  ...
  g.track.setAttribute('stroke', base+(S.paused?'0.20':'0.16')+')');
  g.fill.setAttribute('d', S.spent>0.002 ? _tdArcD(_TD_GAUGE_R,head,_TD_ARC1) : '');
  // Armed sits well below running, so a preview is visibly not a session.
  g.fill.setAttribute('stroke', S.paused ? warm+'0.65)' : base+(S.running?'0.95':'0.45')+')');
  g.fill.setAttribute('filter', S.running ? 'url(#td-sglow)' : '');
}
...
// Cache of today's anchor from Planning › Reflect › "The week, shaped"
...
function _dashRenderNonNeg() {
  ...
  nnEl.innerHTML = `<div class="dash-eyebrow">TODAY'S NON-NEGOTIABLE</div>` + (nn
    ...
    /* The empty state names where the anchor is set, so it takes you there
       rather than reciting a path. It used to say "This week", a Planning tab
       that no longer exists. */
    : `<div class="dash-nn-title muted" data-goto-anchor>Set today's anchor.</div>
       <div class="dash-nn-meta" data-goto-anchor>Planning › Reflect › "The week, shaped" →</div>`);
  nn && nnEl.querySelector('[data-open]')?.addEventListener('click', () => {
    showMainPanel('alltasks');
    setTimeout(() => window.openAtkDetail?.(nn.id), 40);
  });
  nnEl.querySelectorAll('[data-goto-anchor]').forEach(el => el.addEventListener('click', () => {
    showMainPanel('milestones');
    setTimeout(() => window.showPlanTab2?.('reflect'), 40);
  }));
}
...
function _dashRenderCommitments() {
  ...
  const goalsLine = q && q.total
    ? `<div class="dash-goals-line" id="dash-goals-line" title="Open Horizons">
         ...
         <span class="dash-goals-names">${
           q.active
             ? q.goals.filter(g => g.status === 'active').map(g => escHtml(g.title || 'Untitled')).join(' · ')
             : /* No active goal left. Say which way it went rather than "all
                  closed", which read as a contradiction next to "0/1". */
               [q.done    ? `${q.done} achieved` : '',
                q.carried ? `${q.carried} carried forward` : '',
                q.dropped ? `${q.dropped} dropped` : ''].filter(Boolean).join(' · ')
               || 'nothing set'
         }</span>
       </div>`
    : '';
...
function renderPlanHorizons() {
  ...
  const cols = GOAL_HORIZONS.map(h => {
    ...
    /* A goal you carried into the next period is live work, so it belongs with
       the current period rather than filed under "other". It used to land in
       the muted section below the dead original it was created from, which made
       the one thing the user had just done the least visible thing on the page. */
    const isUpcoming = g => g.status === 'active' && String(g.period) > String(nowP[h.id]);
    const current = all.filter(g => g.period === nowP[h.id] || isUpcoming(g));
    const other   = all.filter(g => !(g.period === nowP[h.id] || isUpcoming(g)));
...
function initGoalsModal() {
  ...
  /* Every field writes as it changes, matching the criteria and the link chips,
     which always did. The modal used to mix the two: chips wrote immediately
     while the text fields waited for Save, so Cancel undid half of what you had
     done and nothing said which half. */
  const _goalFieldIds = ['goal-title', 'goal-why', 'goal-horizon', 'goal-period',
                         'goal-parent', 'goal-category'];

  let _goalFieldTimer = null;
  function _goalFlushFields(immediate) {
    const g = GOALS.find(x => x.id === _goalEditId);
    if (!g) return;
    ...
    const patch = {
      title: val('goal-title').trim(),
      why: val('goal-why').trim(),
      horizon, period,
      parentId: val('goal-parent') || null,
      category: val('goal-category') || '',
    };
    Object.assign(g, patch);            // keep the card in step with the modal
    const write = () => updateGoal(g.id, patch).then(() => {
      const hint = document.getElementById('goal-saved');
      if (hint) { hint.classList.add('flash'); setTimeout(() => hint.classList.remove('flash'), 700); }
    });
    clearTimeout(_goalFieldTimer);
    if (immediate) write(); else _goalFieldTimer = setTimeout(write, 500);
  }
  _goalFieldIds.forEach(id => {
    const el = document.getElementById(id);
    if (!el) return;
    el.addEventListener('input',  () => _goalFlushFields(false));
    el.addEventListener('change', () => _goalFlushFields(true));
  });

  document.getElementById('goal-save')?.addEventListener('click', async () => {
    const title = (document.getElementById('goal-title')?.value || '').trim();
    // The one thing still worth blocking on: a goal with no name is unfindable.
    if (!title) { showToast('A goal needs a title.', 'error'); return; }
    _goalFlushFields(true);
    closeOverlay('goal-modal');
    window._refreshPlanDesign?.();
  });
...
function goalsQuarterSummary() {
  ...
  /* Four statuses, and the count has to survive all of them. Reporting
     ...
     contradicted itself. Carried and dropped are reported as what they are. */
  const done    = q.filter(g => g.status === 'done').length;
  const active  = q.filter(g => g.status === 'active').length;
  const carried = q.filter(g => g.status === 'carried').length;
  const dropped = q.filter(g => g.status === 'dropped').length;
  return { period: per, total: q.length, done, active, carried, dropped, goals: q };
}
